...
// Biblioteca de evidencias del colegio: lista PDFs, referencias y feedback por indicador.
type AreaLike = {
  id: string;
  title?: string;
  name?: string;
...
export default function SchoolDocumentsPage() {
  ...
  const {
    ...
    error: contextError,
    repository,
  } = useResolvedSchoolCycleRef({
    schoolId: schoolIdParam,
    cycleId: cycleIdParam,
    allowStoredContextFallback: true,
  });
  ...
  const rows = useMemo<DocumentRow[]>(() => {
    ...
    for (const area of AREAS_SCHEMA as AreaLike[]) {
      for (const indicator of area.indicators || []) {
        ...
        // Mostramos solo indicadores que realmente tienen evidencia visible.
        const hasEvidence = !!response.file || !!response.documentRef.trim() || !!response.comments.trim();
    ...
    return (
      <div className="fni-page-shell">
        <div className="fni-data-panel">
          <h1 className="fni-page-title">Documentos</h1>
          <p className="mt-2 text-sm text-slate-600">Cargando evidencias del ciclo...</p>
        </div>
      </div>
    );
  }

  if (!schoolId || !cycleId || !workspace) {
    return (
      <div className="fni-page-shell">
        <div className="fni-data-panel">
          <h1 className="fni-page-title">Documentos</h1>
          <p className="mt-2 text-sm text-slate-600">
            No se pudo resolver <span className="font-medium">schoolId</span> o{" "}
            <span className="font-medium">cycleId</span>.
          </p>

          <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
            Para esta vista necesitas abrir la ruta con query params o tener un contexto único disponible en la
            fuente activa.
            <br />
            <code className="mt-2 inline-block rounded bg-white px-2 py-1">
              /school/documents?schoolId=sch_1&amp;cycleId=2025
            </code>
          </div>
  ...
  return (
    <div className="fni-page-shell">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="fni-page-title">Documentos</h1>
          <p className="fni-page-subtitle">Biblioteca de evidencias cargadas por indicador.</p>
        </div>

        <div className="flex flex-wrap gap-2">
          <Link
            to={`${ROUTES.school.evaluation}?cycleId=${encodeURIComponent(cycleId)}&schoolId=${encodeURIComponent(
              schoolId
            )}`}
      ...
      {filteredRows.length === 0 ? (
        <div className="fni-empty-state-panel">No hay evidencias que coincidan con los filtros.</div>
      ) : (
                  ...
                  <tr key={row.indicatorId} className="align-top hover:bg-slate-50/60">
                    ...
                    <td className="px-4 py-4">
                      <div className="flex justify-end gap-2">
                        ...
                        <Link
                          to={`${ROUTES.school.evaluation}?cycleId=${encodeURIComponent(
                            cycleId
                          )}&schoolId=${encodeURIComponent(schoolId)}&area=${encodeURIComponent(
                            row.areaId
                          )}&indicator=${encodeURIComponent(row.indicatorId)}`}
